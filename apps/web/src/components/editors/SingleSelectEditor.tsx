...
export default function SingleSelectEditor({ value, onChange, options = [] }: SelectEditorProps) {
              ...
              {rowVirtualizer.getVirtualItems().map((vi) => {
                ...
                return (
                  <div
                    ...
                    style={{ position: 'absolute', top: 0, left: 0, width: '100%', transform: `translateY(${vi.start}px)`, height: vi.size, padding: '8px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', background: isActive ? '#f1f5f9' : 'transparent' }}
                  >
                    {opt.label}
                  </div>
                );
