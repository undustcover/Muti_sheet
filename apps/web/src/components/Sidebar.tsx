...
export const Sidebar: React.FC<Props> = ({ active, onNavigate }) => {
  ...
  const addProject = () => {
    const name = (window.prompt('请输入项目名称') || '').trim();
    if (!name) return;
    setProjects((prev) => [...prev, { id: `p-${prev.length + 1}`, name, tasks: [] }]);
  };

  const addTask = (projectId: string) => {
    const name = (window.prompt('请输入任务名称') || '').trim();
    if (!name) return;
    setProjects((prev) => prev.map(p => p.id === projectId ? { ...p, tasks: [...p.tasks, { id: `t-${p.tasks.length + 1}-${Date.now()}`, name, tables: [] }] } : p));
    setExpandedProjects((e) => ({ ...e, [projectId]: true }));
  };

  const addTable = (projectId: string, taskId: string) => {
